...
DrawingBoard.Utils.isColor = function(string) {
	...
	if(string == 'transparent'){ // --> DentalHorse
		return false;
	}else{
...
DrawingBoard.Board.defaultOpts = {
	...
	size: 1,
	//background: "#fff",
	background: "transparent", // --> DENTALHORSE
	//eraserColor: "background",
	eraserColor: "transparent", // --> DENTALHORSE
	fillTolerance: 100,
	webStorage: 'session',
	droppable: false,
...
DrawingBoard.Board.prototype = {
	...
	_idAnimationFrame: null, // --> DENTALHORSE (ID de acción de AnimationFrame)
	...
	resetBackground: function(background, historize) {
		...
		} else if (background != 'transparent'){ // --> DentalHorse
	...
	setMode: function(newMode, silent) {
		...
		//para activar el modo de pintar a pincel ---> DENTALHORSE
		if (newMode === "brush"){
			this.ev.bind('board:startDrawing', $.proxy(this.brush, this));
		}
	...
	initDrawEvents: function() {
		...
		if (window.requestAnimationFrame) this._idAnimationFrame = requestAnimationFrame( $.proxy(this.draw, this) );
	},

	// --> DENTALHORSE (iniciamos cuando queremos)
	initDraw: function() {

		if (window.requestAnimationFrame) this._idAnimationFrame = requestAnimationFrame( $.proxy(function() { this.draw(); }, this) );
	},

	// --> DENTALHORSE (paramos cuando queremos)
	stopDraw: function() {

		if (window.cancelAnimationFrame && window.requestAnimationFrame) cancelAnimationFrame(this._idAnimationFrame);
	},
	...
	//Nuevo método para implementar los patrones (pinceles) en el canvas ---> DENTALHORSE
	brush: function(e) {
		...
		//Cambiamos el pincel
		var brush = new Image();
		brush.src = this.ctx.brush;

		brush.onload = function() {
			that.ctx.drawImage(brush, e.coords.x-(sizeBrush/2), e.coords.y-(sizeBrush/2), sizeBrush, sizeBrush);
		}
...
DrawingBoard.Control.DrawingMode = DrawingBoard.Control.extend({
	...
	initialize: function() {
		...
		this.$el.on('click', 'button[data-mode]', $.proxy(function(e) {
			...
			this.board.initDraw( newMode );// --> DENTALHORSE (iniciamos el eventos de pintar)
			e.preventDefault();
		}, this));
...
//CONTROLES PERSONALIZADOS PARA DENTALHORSE
DrawingBoard.Control.Dentalhorse = DrawingBoard.Control.extend({
	...
	classCSS:'drawing-board-control-dentalhorse',//clase css de los botones por defecto

	initialize: function() {
		var el = ''
			that = this;

		//cargamos los distintos botones e inicializamos los métodos
		$.each(this.buttons, function(k, v){
			el += '<button class="' + that.classCSS + '-' + k + '" data-mode="brush"><img src="' + that.path + v +'"></button>';

			//Eventos asociados a los botones
			that.$el.on('click', '.' + that.classCSS + '-' + k, $.proxy(function(e) {
				e.preventDefault();

				//cambiamos el modo de pintado
				var newMode = $(e.currentTarget).attr('data-mode');
				this.board.setMode(newMode);
				this.board.stopDraw();

				//establecemos el pincel (imagen a usar)
				that.setBrush(that.path + v);

				//marcamos el botón seleccionado
				that.activeButtonBrush(e.currentTarget);

			}, that));

			//Eventos asociados a los botones
			that.$el.on('blur', '.' + that.classCSS + '-' + k, $.proxy(function(e) {
				e.preventDefault();

				//marcamos el botón seleccionado
				that.activeButtonBrush(null);
	...
	//Cambia el estilo CSS del botón
	activeButtonBrush: function(element){

		$.each($('.drawing-board-control-drawingmode button'), function(k, v){
			$(v).removeClass('active');
		});

		$.each($('.drawing-board-control-dentalhorse button'), function(k, v){
			$(v).removeClass('active');
		});
	...
	//Cambia la forma del pincel
	setBrush: function(url) {

		this.board.ctx.brush = url;
